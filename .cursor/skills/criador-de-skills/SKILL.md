...
description: Cria novas skills do Cursor em PT-BR e melhora skills existentes de forma iterativa. Use SEMPRE que o usuário pedir para criar uma skill, montar uma skill, escrever um SKILL.md, adicionar uma skill ao seu Cursor, melhorar/iterar uma skill, otimizar a descrição (triggering) de uma skill, ou correr avaliações (evals) sobre uma skill. Também aplica quando ele pedir explicitamente @criador-de-skills.
---
...
- Ajudar o usuário a avaliar os resultados (qualitativa e quantitativamente):
  - Enquanto as execuções correm em background, esboçar avaliações quantitativas (asserções) se ainda não existirem. Explicá-las ao usuário.
  - Usar `eval-viewer/generate_review.py` para mostrar os resultados em browser e também as métricas quantitativas.
- Reescrever a skill com base no *feedback* do usuário (e em falhas óbvias detectadas nas métricas).
- Repetir até estar bom.
- Expandir o conjunto de testes e tentar em maior escala.

O teu trabalho ao usar esta skill é perceber onde o usuário está neste processo e ajudá-lo a avançar. Por exemplo, se ele disser "quero fazer uma skill para X", podes ajudar a refinar o âmbito, escrever um rascunho, definir casos de teste, decidir como avaliar, correr os *prompts* e iterar.

Por outro lado, se ele já trouxer um rascunho, vai directo para a fase de avaliar/iterar.

E claro: se o usuário disser "não preciso de evals, só vibra comigo", também faz isso. Sê flexível.

Depois da skill estar "boa" (mas a ordem é flexível), também podes correr o **otimizador da descrição** — há um *script* específico para isso (`scripts/run_loop.py`) que melhora o *triggering* da skill.
...
- **"JSON"** e **"asserção"** precisam de pistas claras de que o usuário conhece os termos antes de os usares sem explicação.

Em dúvida, podes definir brevemente um termo. Não trates o usuário como expert por defeito.
...
Começa por perceber o que o usuário quer. A conversa atual já pode ter o *workflow* que ele quer capturar (ex.: "transforma isto numa skill"). Se for esse o caso, extrai dos turnos anteriores: as ferramentas usadas, a sequência de passos, correções que ele fez, formatos de entrada/saída observados. O usuário valida o que falta.

Perguntas a fazer (sempre):

1. O que esta skill deve permitir o agente fazer?
2. **Quando** deve ela disparar? (que frases/contextos do usuário)
3. Qual o formato esperado de saída?
4. Faz sentido montar **casos de teste** para verificar a skill?

Skills com saídas objectivamente verificáveis (transformações de arquivos, extracção de dados, geração de código, passos fixos) **beneficiam de evals**. Skills com saídas subjectivas (estilo de escrita, design visual) muitas vezes **não precisam**. Sugere o padrão certo, mas deixa o usuário decidir.

### 2) Entrevistar e investigar

Pergunta proactivamente sobre *edge cases*, formatos de entrada/saída, arquivos de exemplo, critérios de sucesso, dependências. **Não escrevas *prompts* de teste antes de estar resolvido.**

Se houver MCPs úteis (procurar documentação, encontrar skills semelhantes, ver *best practices*), investiga em paralelo via *subagents* se possível, ou inline. Vem preparado para reduzir o esforço do usuário.
...
- Referencia arquivos a partir do `SKILL.md` com orientação sobre **quando** lê-los.
- Para arquivos grandes de referência (>300 linhas), inclui um índice no topo.
...
O agente lê apenas o arquivo relevante.

#### Princípio da falta de surpresa

As skills **não devem** conter *malware*, código de exploração, nem nada que comprometa segurança. O conteúdo da skill não pode surpreender o usuário relativamente à sua intenção declarada. Não aceites pedidos para criar skills enganadoras ou desenhadas para facilitar acessos não autorizados, exfiltração de dados ou actividades maliciosas. Coisas como "faz *roleplay* de XYZ" são OK.
...
Usar SEMPRE este template exato:
# [Título]
## Sumário executivo
...
**Exemplo 1:**
Entrada: Adicionada autenticação de usuários com JWT
...
Depois do rascunho, formula 2-3 *prompts* de teste realistas — o tipo de coisa que um usuário real diria. Partilha com o usuário: *"Tenho aqui uns casos de teste para experimentar. Estão certos ou queres acrescentar mais?"* Depois corre-os.
...
```json
{
  ...
  "evals": [
    {
      "id": 1,
      "prompt": "Prompt da tarefa que o usuário escreveria",
...
Esta seção é uma sequência contínua — não pares a meio. **NÃO** uses `/skill-test` ou outras skills de testing.
...
- Arquivos de entrada: <arquivos do eval, ou "nenhum">
- Salvar outputs em: <workspace>/iteration-<N>/eval-<ID>/with_skill/outputs/
- Outputs a salvar: <o que o usuário quer ver — ex.: "o arquivo .docx", "o CSV final">
```
...
Escreve um `eval_metadata.json` para cada caso (com `assertions` ainda vazias). Dá um **nome descritivo** a cada *eval* baseado no que está a testar — não só `eval-0`. Usa esse nome para o diretório também. Se esta iteração introduzir *evals* novos ou modificados, cria estes arquivos para cada novo `eval-<...>/`.
...
Não fiques só à espera — usa este tempo. Esboça asserções **quantitativas** para cada caso e explica-as ao usuário. Se já existirem em `evals/evals.json`, revê-as e explica o que verificam.
...
Atualiza os `eval_metadata.json` e o `evals/evals.json` com as asserções já desenhadas. Explica ao usuário o que vai ver no *viewer* — outputs qualitativos **e** *benchmark* quantitativo.
...
2. **Agregar para benchmark** — corre o script de agregação a partir do diretório da skill:
   ...
   Produz `benchmark.json` e `benchmark.md` com taxa de aprovação, tempo e *tokens* por configuração, com média ± desvio padrão e *delta*. Se gerares `benchmark.json` manualmente, vê `references/schemas.md` para o esquema exato. **Põe sempre a versão *with_skill* antes da *baseline* correspondente.**

3. **Passagem de analista** — lê o *benchmark* e identifica padrões que as estatísticas agregadas escondem. Vê `agents/analyzer.md` (seção "Analisar resultados de *benchmark*") — coisas como asserções que passam sempre (não discriminam), *evals* com alta variância (provavelmente *flaky*) e *trade-offs* entre tempo/tokens e qualidade.
...
4. **Abrir o *viewer*** com dados qualitativos e quantitativos:
   ...
   **Ambientes sem display:** se `webbrowser.open()` não estiver disponível ou não houver *display*, usa `--static <output_path>` para gerar um arquivo HTML standalone em vez de servidor. O *feedback* é descarregado como `feedback.json` quando o usuário clica em "Submit All Reviews". Depois copia esse `feedback.json` para o diretório do *workspace* para a próxima iteração o ler.

   Nota: usa sempre `generate_review.py` para criar o *viewer* — não escrevas HTML à mão.

5. **Diz ao usuário**, em PT, algo do tipo: *"Abri os resultados no teu browser. Tens dois separadores — 'Outputs' permite navegar pelos casos de teste e deixar comentários; 'Benchmark' mostra a comparação quantitativa. Quando terminares, volta aqui e diz."*
...
- **Output**: os arquivos que a skill produziu, renderizados *inline* sempre que possível.
- **Previous Output** (iteração 2+): seção recolhida com o *output* da iteração anterior.
- **Formal Grades** (se o *grading* foi feito): seção recolhida com pass/fail das asserções.
- **Feedback**: uma caixa de texto que auto-grava enquanto o usuário escreve.
...
Quando o usuário disser que terminou, lê `feedback.json`:
...
*Feedback* vazio significa "ficou bom". Concentra as melhorias nos casos onde o usuário apontou problemas específicos.
...
O coração do *loop*. Já corre-este os casos, o usuário revê os resultados e agora vais melhorar.

### Como pensar nas melhorias

1. **Generaliza a partir do feedback.** O objetivo é criar skills usadas milhões de vezes, com muitos *prompts* diferentes. Aqui itera-se sobre uns poucos exemplos porque é rápido para o usuário avaliar. Mas se a skill **só** funcionar para esses exemplos, é inútil. Em vez de mudanças *fiddly* específicas, ou `MUSTs` opressivos, tenta novas metáforas, novos padrões de trabalho. É barato experimentar.
...
3. **Explica o porquê.** Tenta sempre explicar **porque** estás a pedir cada coisa. Hoje os LLMs são *smart*. Têm boa *theory of mind* e, com um bom *harness*, vão além de seguir instruções à letra. Mesmo se o feedback for terso ou frustrado, tenta entender o que o usuário quer realmente. Se te apanhas a escrever `SEMPRE` ou `NUNCA` em maiúsculas, ou a usar estruturas super-rígidas, é um *yellow flag* — reformula e explica o porquê.
...
Tira o teu tempo. Escreve um rascunho da revisão, deixa arrefecer, melhora. Entra na cabeça do usuário e percebe o que ele quer.
...
4. Espera o usuário rever e dizer "terminei".
...
É opcional, precisa de *subagents*, e a maioria dos usuários não precisa. O *loop* humano costuma chegar.

---

## Optimização da descrição

A descrição (frontmatter) é o que decide se o agente invoca a skill. Depois de criar/melhorar uma skill, **oferece** otimizar a descrição.
...
```json
[
  {"query": "prompt do usuário", "should_trigger": true},
  {"query": "outro prompt", "should_trigger": false}
]
```

As *queries* têm de ser **realistas** — coisas que um usuário real de Cursor escreveria. Não pedidos abstractos, mas concretos: caminhos de arquivo, contexto pessoal (cargo do usuário, situação), nomes de colunas e valores, nomes de empresas, URLs. Um bocadinho de história. Alguns em *lowercase*, abreviações, *typos*, fala casual. Diferentes comprimentos. Foco em casos-limite.
...
Para **should-trigger** (8-10): cobertura — diferentes formulações da mesma intenção (formais, casuais). Casos onde o usuário não nomeia explicitamente a skill mas precisa dela. Casos pouco comuns. Casos onde compete com outra skill mas devia ganhar.
...
### Passo 2: Rever com o usuário

Apresenta o conjunto ao usuário usando o template HTML:
...
2. Substitui os *placeholders*:
   ...
   - `__SKILL_DESCRIPTION_PLACEHOLDER__` → descrição atual.
3. Grava como `eval_review_<nome>.html` num diretório temporário e abre no browser.
4. O usuário edita *queries*, alterna *should-trigger*, adiciona/remove entradas, e clica "Export Eval Set".
5. O arquivo é descarregado para a pasta *Downloads* — confirma a versão mais recente.

Este passo importa — *queries* más → descrição má.

### Passo 3: Correr o *loop* de otimização

Diz ao usuário: *"Isto demora um pouco. Vou correr em background e ir verificando."*
...
Usa o `model-id` da sessão atual (o que está a alimentar o assistente neste momento) para que o teste de *triggering* corresponda ao que o usuário experiencia.

Enquanto corre, vai verificando o *output* periodicamente e dá updates de iteração e *scores*.

Este script lida com tudo: divide o conjunto em 60% treino / 40% teste, avalia a descrição atual (3 vezes por *query* para fiabilidade), chama o agente para propor melhorias com base nas falhas, e re-avalia em treino + teste, até 5 iterações. No fim abre um relatório HTML e devolve JSON com `best_description` — escolhida pelo *score* de **teste** (não treino) para evitar *overfit*.
...
Pega `best_description` do JSON e atualiza o `SKILL.md`. Mostra ao usuário antes/depois e reporta os *scores*.
...
Aponta o usuário para o arquivo `.skill` resultante para instalar.
...
- **Rever**: se não conseguires abrir browser, **salta o *viewer***. Apresenta os resultados directamente na conversa. Para cada caso, mostra *prompt* + *output*. Se o output for um arquivo (.docx, .xlsx), grava no *filesystem* e diz onde está. Pede feedback inline.
...
- **Atualizar skill existente**: o usuário pode estar a pedir update, não criação. Nesse caso:
...
- Avaliar com o usuário:
  - Criar `benchmark.json` e correr `eval-viewer/generate_review.py` para o usuário rever.
