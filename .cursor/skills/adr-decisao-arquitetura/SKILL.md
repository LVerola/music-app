...
description: Cria Architecture Decision Records (ADRs) em PT-BR seguindo o formato Michael Nygard estendido — contexto, opções consideradas com prós/contras, decisão, consequências (positivas e negativas), riscos, plano de revisão. Use SEMPRE que o usuário pedir para documentar uma decisão técnica, registrar uma escolha de arquitetura, escrever um ADR, comparar tecnologias para uma decisão, ou pedir @adr-decisao-arquitetura. Também aplique quando descrever uma decisão técnica importante que precisa ser registrada para a equipe.
---

# ADR — Architecture Decision Record

Skill para documentar **decisões arquiteturais importantes** de forma que o "futuro" da equipe entenda **o quê** foi decidido, **por que**, **o que foi considerado e rejeitado**, e **quando revisar**.
...
- "Vamos adotar feature flags? Qual ferramenta?"
- "Como autenticar serviços internos?"
- "Onde colocar a fila — RabbitMQ, SQS, Kafka?"
- "Adotar monorepo ou repositórios separados?"
...
1. **Pergunta exata** que está sendo decidida? (Uma só, atómica.)
2. **Forças em jogo**: requisitos, restrições, dores atuais, custos.
3. **Quem decide**: arquiteto? Tech lead? Equipe? Engenharia + produto?
4. **Quando precisa estar decidido**?
5. **Alternativas em cima da mesa** (no mínimo 2-3 — se há só uma, não é decisão, é proposta).
6. **Critérios de avaliação**: como a equipe vai comparar as alternativas?
7. **O que já foi descartado** e por quê?
8. **Decisão preliminar**, se houver, e quem a defende.

Se o usuário trouxer só "queremos adotar X", **pare** e force-o a explicitar pelo menos uma alternativa. ADR sem alternativa é decoração.

---

## 2. Template obrigatório

Salve em `.docs/Documentacao/ADRs/<numero>-<slug>.md` (numeração sequencial: `0001`, `0002`...). Se a pasta não existir, crie.

```markdown
# ADR-<NNNN>: <Decisão em uma frase, no infinitivo>

> **Situação**: Proposto | Em discussão | Aceito | Rejeitado | Superado por ADR-<X> | Obsoleto
> **Data**: AAAA-MM-DD
> **Decisores**: <papéis ou nomes>
> **Consultados**: <equipes/papéis envolvidos no debate>
...
- O que está acontecendo agora (estado atual).
...
> Bom contexto: alguém de fora consegue entender **por que** a equipe está parada para decidir isto **agora**, sem precisar ler outro documento.
...
- **Maturidade da equipe**: <ex.: ninguém na equipe conhece tecnologia Y>
- **Reversibilidade**: <ex.: depois de adotar, custa N semanas para sair>
- **Conformidade**: <ex.: LGPD exige dados em região X>
- **Time-to-market**: <ex.: precisamos lançar em 6 semanas>
- **Compatibilidade com plataforma atual**: ...
...
<1-2 parágrafos explicando o porquê — referenciando as forças da seção 2 e os prós/contras da seção 3. Seja específico:>
...
- <ex.: equipe vai precisar de treinamento na ferramenta X>
...
- <ex.: depois de 6 meses de uso, com retrospectiva da equipe>
...
Quando o usuário pedir "compara X vs Y vs Z", monte uma matriz **antes** de decidir. Pesos opcionais (1-5).

| Critério (peso) | Opção A | Opção B | Opção C |
|---|---|---|---|
| Performance (5) | 4 | 5 | 3 |
| Custo operacional (4) | 3 | 2 | 5 |
| Maturidade na equipe (3) | 5 | 1 | 4 |
...
| **Em discussão** | Aberto a feedback da equipe antes de votar |
...
- **Trade-off explícito**: toda decisão tem **algo de mau**. Se a sua seção 5.2 está vazia, releia.
...
|---|---|---|
| Uma "opção" só | Seção 3 tem 1 entrada | Forçar **pelo menos 2** alternativas |
| Decisão sem contras | Seção 5.2 vazia | Listar *trade-offs* honestos |
| "Performance melhor" sem número | Texto vago | Substituir por benchmark / SLA |
| Critério inventado pra justificar pré-conclusão | Matriz com 1 critério desproporcional | Pesos calibrados com equipe |
...
| Situação "Aceito" sem decisores nomeados | Cabeçalho incompleto | Identificar **quem** decidiu |
| Sem plano de revisão (seção 7) | Decisão eternizada | Adicionar gatilhos concretos |
...
Documentacao/
└── ADRs/
    ├── 0001-adotar-postgres-como-bd-primario.md
...
- Frontmatter / cabeçalho exatamente como o template.

---

## 9. Pós-entrega

- Sugira que o usuário compartilhe o ADR no canal técnico para feedback antes de mover para "Aceito".
- Se o ADR fizer parte de US, anexe link à US.
- Lembre que ADRs aceitos viram "fonte de verdade" — se uma regra ou orientação mudar por causa do ADR, atualize também a regra.

## Notas de idioma

- Todo o ADR em **português brasileiro** (`AGENTS.md` §1).
- Sem português europeu e sem inglês de prosa (`Status` → `Situação`).
